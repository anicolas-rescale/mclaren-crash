#!/usr/bin/env bash
# Post-job tile launcher — AI Surface Extractor [DEV] (EU)
# Tile Command:  bash /enc/tmp/run_ai.sh
...
# McLaren scorecard needs disp + PSA probe overlays — not mesh stress/strain.
export NODAL_SCALARS=none
export FRAME_POLICY=all
# Leave MAX_FRAMES unset for full transient; if Emerald 16GB OOMs, set e.g. 12.

# digest sha256:cd35d24d00f51be932c10abc2007a897c44152a4c6a7b043cfa7c98fcdd9ff50
IMG="631046354827.dkr.ecr.eu-central-1.amazonaws.com/deprod-rescale-automation-images-customer:automation-ai-extractor-0.1.12"
STAGE="/enc/tmp/automation-ai-extractor"
...
env | grep -E '^RESCALE.*|^USER|^HOME' > /enc/tmp/mclaren-ai.env.list

rm -rf "$STAGE/python_venv"
i=1
while [ "$i" -le "$MAX_ATTEMPTS" ]; do
  log "stage attempt $i/$MAX_ATTEMPTS"
  if podman run --rm --userns=keep-id --env-file /enc/tmp/mclaren-ai.env.list -v /enc/tmp:/enc/tmp "$IMG"; then
...
log "launching AI on host from $WORK_ROOT"
cd "$WORK_ROOT"
"$VENV_PY" -m automation.main
log "AI extractor finished"
